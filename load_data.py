        ...
        for data in sorted(os.listdir(testdataset_dir)):
            ...
            for fr in sorted(os.listdir(data_dir)):
                if fr[-8] != 'd' and fr[-8] != 'I':
                    ...
                    image_name = '{0}/{1}_frame{2}.nii.gz'.format(data_dir, data, fr[-9:-7]) # test_data/patient001/patient_frame01.nii.gz
...
def read_data(raw_train_data_dir,process_data_dir,image_size,n_slice,is_csn=False):
    if not os.path.exists(process_data_dir):
        os.makedirs(process_data_dir)

    train_data_list = select_training_data(raw_train_data_dir)
...
def adjust_data_3D(images_train,masks_train,images_validation,masks_validation,image_size,n_slice):
  ...
  images_train_3D = np.zeros((176,image_size, image_size,n_slice))
  masks_train_3D = np.zeros((176,image_size, image_size,n_slice))
  images_validation_3D = np.zeros((20,image_size, image_size,n_slice))
  masks_validation_3D = np.zeros((20,image_size, image_size,n_slice))
