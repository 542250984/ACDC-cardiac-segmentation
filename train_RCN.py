...
from experiments import exp_RCN as exp
...
if __name__ == '__main__':
    ...
    dataset = read_data(exp.raw_training_data_dir, exp.process_data_dir, exp.image_size, exp.n_slice, is_csn=False)
    images_train = dataset['images_train']
    gts_train = dataset['gts_train']
    images_validation = dataset['images_validation']
    gts_validation = dataset['gts_validation']

    # Numpy Array
    images_train_2D, gts_train_2D, images_validation_2D, gts_validation_2D = adjust_data(images_train,
                                                                                                 gts_train,
                                                                                                 images_validation,
                                                                                                 gts_validation, exp.image_size)
    print(images_train_2D.shape,gts_train_2D.shape)

    with tf.Graph().as_default():

        image_pl = tf.placeholder(tf.float32, shape=[None, None, exp.image_size], name='image_pl')
        segt_pl = tf.placeholder(tf.int32, shape=[None, None, exp.image_size], name='segt_pl')
        ...
        logits_segt = RCN(image_pl, exp.segt_class, training=training_pl,n_filter=[96, 96, 96,96, 96])

        loss_segt, accuracy_segt, dice_0, dice_1, dice_2, dice_3, dice_all, pred_segt = tf_loss_accuary_2D_channel_first(logits_segt,segt_pl,exp.segt_class)
        ...
        for epoch in range(init_epoch,  exp.train_epoch):
            ...
            data_len = images_train_2D.shape[0] #1680
            n_per_epoch = int(data_len / exp.train_batch_size) #168
            ...
            for iteration in range(n_per_epoch):
                ...
                images = np.zeros((exp.train_batch_size, exp.image_size, exp.image_size))
                segt_labels = np.zeros((exp.train_batch_size, exp.image_size, exp.image_size))

                for v in range(exp.train_batch_size):
                    images[v, :, :] = images_train_2D[image_in_batch_index[v] , :, :]
                    segt_labels[v, :, :] = gts_train_2D[image_in_batch_index[v] , :, :]

                images, segt_labels = data_augmentation(images, segt_labels,  flip=True, gamma=True)
            ...
            data_len_val = images_validation_2D.shape[0] #210
            n_per_epoch_val = int(data_len_val / exp.validation_batch_size) #21
            ...
            for iteration_val in range(n_per_epoch_val):
                ...
                valid_images = np.zeros((exp.validation_batch_size, exp.image_size, exp.image_size))
                valid_labels = np.zeros((exp.validation_batch_size, exp.image_size, exp.image_size))

                for v in range(exp.validation_batch_size):
                    valid_images[v, :, :] = images_validation_2D[val_image_in_batch_index[v], :, :]
                    valid_labels[v, :, :] = gts_validation_2D[val_image_in_batch_index[v], :, :]
